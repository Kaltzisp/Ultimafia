...
async function cacheUserInfo(userId, reset) {
  ...
  if (!exists || reset) {
    ...
    await client.set(`user:${userId}:info:avatar`, user.avatar);
    await client.set(`user:${userId}:info:nameChanged`, user.nameChanged);
    await client.set(`user:${userId}:info:bdayChanged`, user.bdayChanged);
    await client.set(`user:${userId}:info:birthday`, user.birthday);
    await client.set(
...
async function registerGameServer(port) {
  await client.sAdd("gameServers", port);
}

async function removeGameServer(port) {
  await client.sRem("gameServers", port);
}
